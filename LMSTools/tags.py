class LMSTags(object):
    ...
    :const ARTWORK_TRACK_ID: Identifier of the album track used by the server to display the album's artwork.
    Not listed if artwork is not available for this album.
    ...
    :const GENRE_ID_LIST: Genre IDs, separated by commas (only useful if the server is set to handle multiple items
    in tags).
    :const GENRE_ID: Genre ID. Only if known.
    :const GENRE_LIST: Genre names, separated by commas (only useful if the server is set to handle multiple items
    in tags).
